...
class User(SQLModel, table=True):
    ...
    email: str
    password: str
    name: str = Field(default="Anonymous")  # Add name field for audit purposes
...
class Student(SQLModel, table=True):
    ...
    created_by: str = Field(default="Anonymous")  # Audit trail
    updated_at: Optional[datetime] = None
    updated_by: Optional[str] = Field(default="Anonymous")  # Audit trail
...
async def login(request: LoginRequest, response: Response):
    ...
    with Session(engine) as session:
        try:
            ...
            query = text("SELECT id, email, password, name FROM usertable")
            ...
            for user in all_users:
                user_id, user_email, user_password, user_name = user
...
async def debug_users():
    ...
    with Session(engine) as session:
        query = text("SELECT id, email, name FROM usertable ORDER BY id")
        result = session.execute(query)
...
# ========== STUDENT ROUTES ==========
@app.post("/students/")
async def create_student(request: StudentCreate, request_obj: Request):
    """Create a new student with audit trail"""
    ...
    creator_name = session_data.get("name", "Anonymous")
    print(f"📝 Creating student: {request.name} ({request.email}) by {creator_name}")
    ...
    with Session(engine) as session:
        ...
        # Create student using raw SQL with audit trail
        insert_query = text("""
            INSERT INTO student (name, email, created_at, created_by, updated_by)
            VALUES (:name, :email, :created_at, :created_by, :updated_by)
...
async def list_students():
    """List all students"""
    with Session(engine) as session:
        query = text("SELECT id, name, email, created_at, created_by, updated_by FROM student ORDER BY created_at DESC")
        result = session.execute(query)
...
async def update_student(student_id: int, request: dict, request_obj: Request):
    """Update a student with audit trail"""
    session_token = request_obj.cookies.get("session_token")
    session_data = get_session(session_token)
    ...
    updater_name = session_data.get("name", "Anonymous")
    print(f"📝 Updating student {student_id} by {updater_name}")
    ...
    with Session(engine) as session:
        try:
            ...
            update_fields = []
            params = {"student_id": student_id, "updated_by": updater_name, "updated_at": datetime.utcnow()}
            ...
            # Add audit fields
            update_fields.extend(["updated_by = :updated_by", "updated_at = :updated_at"])
            ...
            print(f"✅ Updated student {student_id} by {updater_name}:", student_dict)
            return student_dict
...
async def delete_student(student_id: int, request_obj: Request):
    """Delete a student"""
    session_token = request_obj.cookies.get("session_token")
    session_data = get_session(session_token)
    ...
    deleter_name = session_data.get("name", "Anonymous")
    print(f"🗑️ Deleting student {student_id} by {deleter_name}")
    ...
    with Session(engine) as session:
        try:
            ...
            update_todos_query = text("""
                UPDATE todo
                SET student_id = NULL, updated_by = :updated_by, updated_at = :updated_at
                WHERE student_id = :student_id
            """)
            session.execute(update_todos_query, {
                "student_id": student_id,
                "updated_by": deleter_name,
            ...
            return {
                "success": True,
                "message": f"Student {student_id} deleted successfully by {deleter_name}",
...
# ========== TODO ROUTES ==========
@app.post("/todos/")
async def create_todo(request: TodoCreate, request_obj: Request):
    """Create a new todo with audit trail"""
    session_token = request_obj.cookies.get("session_token")
    session_data = get_session(session_token)
    ...
    creator_name = session_data.get("name", "Anonymous")
    print(f"📝 Creating todo: {request.title} by {creator_name}")

    with Session(engine) as session:
        # Create todo using raw SQL with audit trail
        insert_query = text("""
            INSERT INTO todo (title, description, priority, status, due_date, student_id, created_at, created_by, updated_by)
            VALUES (:title, :description, :priority, :status, :due_date, :student_id, :created_at, :created_by, :updated_by)
            RETURNING id, title, description, priority, status, due_date, student_id, created_at, created_by
        """)
...
async def list_todos():
    ...
    with Session(engine) as session:
        query = text("""
            SELECT id, title, description, priority, status, due_date, student_id, created_at, created_by, updated_by, completed_at
            FROM todo
            ORDER BY created_at DESC
        """)
        result = session.execute(query)
...
async def update_todo(todo_id: int, request: dict, request_obj: Request):
    """Update a todo - SQLAlchemy 2.0 compatible with audit trail"""
    session_token = request_obj.cookies.get("session_token")
    session_data = get_session(session_token)
    ...
    updater_name = session_data.get("name", "Anonymous")
    print(f"📝 Updating todo {todo_id} by {updater_name}")
    ...
    with Session(engine) as session:
        try:
            ...
            update_fields = []
            params = {"todo_id": todo_id, "updated_by": updater_name, "updated_at": datetime.utcnow()}
            ...
            print(f"✅ Updated todo {todo_id} by {updater_name}:", todo_dict)
            return todo_dict
...
# ========== CHATBOT API ENDPOINT WITH AI INTEGRATION ==========
@app.post("/chat")
async def chat_endpoint(request: ChatRequest, request_obj: Request):
    """Enhanced chat endpoint for the AI assistant with multiple AI service support"""
    session_token = request_obj.cookies.get("session_token")
    session_data = get_session(session_token)
    ...
    try:
        ...
        ai_service = os.getenv("AI_SERVICE", "openai")  # Default to OpenAI
        
        # Convert message to lowercase for processing
        user_msg_lower = user_message.lower()

        # Define responses based on keywords if no AI service is configured
        if not os.getenv("OPENAI_API_KEY") and not os.getenv("ANTHROPIC_API_KEY") and not os.getenv("GOOGLE_API_KEY"):
            ...
            if any(word in user_msg_lower for word in ["hello", "hi", "hey", "greetings"]):
                response = f"Hello {user_name}! I'm your AI assistant for managing tasks. How can I help you today?"
            elif any(word in user_msg_lower for word in ["help", "assist", "support"]):
                response = (f"I can help you manage your todos and students. "
                           f"You can ask me to create, update, or check tasks. "
                           f"For example, you can say 'create a new todo', 'show my tasks', or 'add a student'.")
            elif any(word in user_msg_lower for word in ["todo", "task", "work", "assignment"]):
                response = (f"I can help you manage your todos. You can ask me to create a new task, "
                           f"mark a task as complete, or view your tasks. "
                           f"Try saying 'create a new todo called [task name]' or 'show my todos'.")
            elif any(word in user_msg_lower for word in ["student", "class", "enroll"]):
                response = (f"I can help you manage students. You can ask me to create a new student "
                           f"or check student information. Try saying 'add a student named [name] with email [email]'.")
                ...
                if any(word in user_msg_lower for word in ["todo", "task", "work", "assignment"]):
                    response = (f"To create a new todo, go to the 'Todos' section and click 'Add New Todo'. "
                               f"You can specify the title, description, priority, and assign it to a student.")
                elif any(word in user_msg_lower for word in ["student", "class", "person"]):
                    response = (f"To create a new student, go to the 'Students' section and click 'Add New Student'. "
                               f"You'll need to provide the student's name and email address.")
                else:
                    response = (f"You can create new todos or students through the interface. "
                               f"Would you like to create a new todo or student?")
            elif any(word in user_msg_lower for word in ["complete", "done", "finish"]):
                response = (f"You can mark tasks as complete in the todos section of the application. "
                           f"Find the task you want to mark as complete and click the checkbox next to it.")
            elif any(word in user_msg_lower for word in ["view", "see", "show", "list"]):
                response = (f"You can view your todos and students in their respective sections of the application. "
                           f"Click on 'Todos' or 'Students' in the navigation menu to see all items.")
                ...
                response = (f"To create a new todo, navigate to the 'Todos' section and click 'Add New Todo'. "
                           f"Fill in the title, description, priority level, and assign it to a student if needed.")
            elif "create student" in user_msg_lower or "add student" in user_msg_lower:
                response = (f"To create a new student, navigate to the 'Students' section and click 'Add New Student'. "
                           f"Enter the student's name and email address, then click 'Create'.")
            elif "delete" in user_msg_lower and any(word in user_msg_lower for word in ["todo", "task"]):
                response = (f"To delete a todo, go to the 'Todos' section, find the task you want to delete, "
                           f"and click the delete button associated with that task.")
            elif "delete" in user_msg_lower and any(word in user_msg_lower for word in ["student"]):
                response = (f"To delete a student, go to the 'Students' section, find the student you want to delete, "
                           f"and click the delete button associated with that student. Note that deleting a student "
                           f"will remove them from any assigned tasks but won't delete the tasks themselves.")
            elif "update" in user_msg_lower or "edit" in user_msg_lower:
                response = (f"To update a todo or student, go to the respective section, find the item you want to edit, "
                           f"and click the edit button. Make your changes and save them.")
            elif "priority" in user_msg_lower:
                response = (f"Todos can have three priority levels: Low, Medium, or High. "
                           f"When creating or editing a todo, you can select the appropriate priority level.")
            elif "status" in user_msg_lower:
                response = (f"Todos have three status options: 'To Do', 'In Progress', and 'Done'. "
                           f"You can update the status as you work on your tasks.")
            elif "due date" in user_msg_lower:
                response = (f"When creating or editing a todo, you can optionally set a due date. "
                           f"This helps you keep track of upcoming deadlines.")
            else:
                response = (f"I received your message: '{user_message}'. I'm your AI assistant for managing tasks. "
                           f"You can ask me about creating, viewing, or managing your todos and students. "
                           f"For example, try asking about creating a new todo or adding a student.")
        ...
        if ai_service == "openai" and os.getenv("OPENAI_API_KEY"):
            ...
            # Fallback to rule-based responses if no service is properly configured
            response = f"I'm sorry, but I couldn't connect to the AI service. Please make sure your API key is set correctly. You asked: '{user_message}'"

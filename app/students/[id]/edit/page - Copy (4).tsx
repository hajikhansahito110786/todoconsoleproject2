...
export default function EditStudentPage() {
  ...
  // Initialize with empty values to avoid undefined
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        ...
        // Set form data with actual values or fallback to empty strings
        setFormData({
          name: data.name || '',
          email: data.email || ''
        });
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                type="text"
                name="name"
                value={formData.name || ''}  // ✅ Added fallback to empty string
            ...
            <div>
              ...
              <input
                type="email"
                name="email"
                value={formData.email || ''}  // ✅ Added fallback to empty string
                onChange={handleChange}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                required
              />

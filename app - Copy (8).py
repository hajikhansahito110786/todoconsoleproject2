...
# SQLModel User table - SIMPLE VERSION (development only)
class User(SQLModel, table=True):
    ...
    name: Optional[str] = None
    password: str  # Store plain password for development
    is_active: bool = Field(default=True)
    created_at: datetime = Field(default_factory=datetime.utcnow)

# ========== SIMPLE AUTH UTILITY FUNCTIONS ==========
# IMPORTANT: This is for development only. Use proper hashing in production!
...
def get_password_hash(password):
    """Simple password storage (development only)"""
    # In production, use: return pwd_context.hash(password)
    return password
...
async def lifespan(app: FastAPI):
    ...
    with Session(engine) as session:
        ...
        if not admin_user:
            # Create default admin user with SIMPLE password storage
            admin_user = User(
...
async def sign_up(request: SignUpRequest):
    with Session(engine) as session:
        ...
        user = User(
            email=request.email,
            name=request.name,
            password=request.password,  # Plain password for development
            is_active=True
        )
        session.add(user)
        session.commit()
...
if __name__ == "__main__":
    ...
    print("✅ SIMPLE Authentication System Added (Development)")
